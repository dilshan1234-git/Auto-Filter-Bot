...
# Bot information
API_ID = environ.get('API_ID', '14631157')
if len(API_ID) == 0:
...
if len(API_HASH) == 0:
    ...
    exit()
BOT_TOKEN = environ.get('BOT_TOKEN', '8219692010:AAFiMHsdtT06KK9DfSvGGmWRa_X3k0pZOD0')
if len(BOT_TOKEN) == 0:
...
# Bot Admins
ADMINS = environ.get('ADMINS', '5380833276')
if len(ADMINS) == 0:
...
# Channels
INDEX_CHANNELS = [int(index_channels) if index_channels.startswith("-") else index_channels for index_channels in environ.get('INDEX_CHANNELS', '-1003513311497').split()]
if len(INDEX_CHANNELS) == 0:
    logger.info('INDEX_CHANNELS is empty')
LOG_CHANNEL = environ.get('LOG_CHANNEL', '-1003513311497')
...
# support group
SUPPORT_GROUP = environ.get('SUPPORT_GROUP', '-1002706396514')
...
# MongoDB information
DATA_DATABASE_URL = environ.get('DATA_DATABASE_URL', "mongodb+srv://Dilshan:<db_password>@moviecluster.obxil1i.mongodb.net/?appName=Moviecluster")
if len(DATA_DATABASE_URL) == 0:
    logger.error('DATA_DATABASE_URL is missing, exiting now')
    exit()
FILES_DATABASE_URL = environ.get('FILES_DATABASE_URL', "mongodb+srv://Dilshan:<db_password>@moviecluster.obxil1i.mongodb.net/?appName=Moviecluster")
if len(FILES_DATABASE_URL) == 0:
    logger.error('FILES_DATABASE_URL is missing, exiting now')
    exit()
SECOND_FILES_DATABASE_URL = environ.get('SECOND_FILES_DATABASE_URL', "mongodb+srv://Dilshan:<db_password>@moviecluster.obxil1i.mongodb.net/?appName=Moviecluster")
if len(SECOND_FILES_DATABASE_URL) == 0:
    logger.info('SECOND_FILES_DATABASE_URL is empty')
DATABASE_NAME = environ.get('DATABASE_NAME', "Moviecluster")
...
BIN_CHANNEL = environ.get("BIN_CHANNEL", "-1003513311497")
